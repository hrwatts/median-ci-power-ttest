...
If the true mean satisfies $\mu=\mu_0+\Delta$ with $\Delta>0$, define
\[
\pi_{n,\alpha}(x)=1-G_{\nu,\sqrt{n}x}\!\left(t_{1-\alpha,\nu}\right),
\]
where $G_{\nu,\delta}$ is the cumulative distribution function of the noncentral t distribution with $\nu$ degrees of freedom and noncentrality parameter $\delta$ \citep{tarasinska2005power}. Then power at raw shift $\Delta$ and scale $\sigma$ is
\[
\pi_{n,\alpha}\!\left(\frac{\Delta}{\sigma}\right).
\]
...
\left[\sqrt{\frac{\nu S^2}{B}},\sqrt{\frac{\nu S^2}{A}}\right].
\]
Because power decreases in $\sigma$ for fixed $\Delta>0$, the observable power interval is
\[
C^{\mathrm{obs}}_{A,B}(S;\Delta)
=
\left[
\pi_{n,\alpha}\!\left(\frac{\Delta}{S}\sqrt{\frac{A}{\nu}}\right),
\pi_{n,\alpha}\!\left(\frac{\Delta}{S}\sqrt{\frac{B}{\nu}}\right)
\right].
\]

For design-stage analysis of random interval length, use $S/\sigma=\sqrt{Q/\nu}$ and the scale-free index $\kappa=\Delta/\sigma$ to rewrite the same random endpoints as
\[
...
\begin{proposition}
For constants $A$ and $B$ chosen before observing $S$, with $\Pr(A\le Q\le B)=1-\gamma$, the interval $C^{\mathrm{obs}}_{A,B}(S;\Delta)$ has exact coverage:
\[
\Pr\!\left\{\pi_{n,\alpha}(\Delta/\sigma)\in C^{\mathrm{obs}}_{A,B}(S;\Delta)\right\}=1-\gamma.
\]
\end{proposition}
...
\[
\pi_{n,\alpha}(\Delta/\sigma)\in C^{\mathrm{obs}}_{A,B}(S;\Delta)
\iff
...
\noindent\textit{Remark.} Following Chakraborti et al., the numerical design problem is indexed by a standardized shift. This is a scale-free tabulation convention, not an assumption that the population standard deviation is known in the data analysis. The endpoint allocation is fixed before observing the sample standard deviation. Exact coverage is pointwise in each fixed $\kappa$, and it fails if $\kappa$ or the allocation $p$ is selected after observing $S$.
...
The fixed-endpoint architecture is unchanged from \citet{chakraborti2012shortest}: choose a fixed allocation $p$, map to fixed chi-square endpoints $(A_p,B_p)$, obtain exact coverage, then optimize a length functional over the pivot randomness. The sole methodological change in this note is replacing the expected-length criterion with a median-length criterion.
...
\label{tab:optima}
\resizebox{\textwidth}{!}{\input{../results/table_optima.tex}}
\end{table}
...
\label{tab:lengths}
\resizebox{\textwidth}{!}{\input{../results/table_lengths.tex}}
\end{table}
...
\label{tab:mcsens}
\resizebox{\textwidth}{!}{\input{../results/table_mc_sensitivity.tex}}
\end{table}
...
\label{tab:mc}
\resizebox{0.9\textwidth}{!}{\input{../results/table_monte_carlo.tex}}
\end{table}

As a defensive software check, the repository also includes \texttt{scripts/posthoc\_allocation\_failure\_demo.py}, which applies an invalid data-adaptive rule that selects $p$ after observing $S$. The script reports empirical coverage for that post-hoc rule alongside a valid fixed-allocation baseline and is included only to demonstrate why post-hoc allocation is not exact-coverage inference.
...
The method developed here should also be understood alongside competing approaches that build confidence intervals for noncentrality parameters directly. The transformed-$\sigma$ architecture used here is chosen deliberately for a narrow finite-sample target: fixed raw shift $\Delta$, unknown $\sigma$, fixed endpoint allocation, and exact pivot-based coverage. Noncentrality-parameter approaches are adjacent comparison work rather than part of the core inferential claim of this note.

The present method is indexed by a prespecified standardized alternative and its exact-coverage claim is pointwise in that index; it is not a replacement for prospective power planning or for post-data effect estimation. Its more modest role is to provide an exact low-dimensional finite-sample benchmark for how power intervals behave once a standardized target has already been fixed.
